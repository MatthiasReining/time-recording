...
export default class AbstractAttrRenderer extends AbstractWebComponent {
    ...
    _updateValue(value) {
        const detail = { newValue: value, oldValue: this.container[this.attrDef.key] };
        this.container[this.attrDef.key] = value;
        this.refreshContext();
    ...
    _renderTooltip(tooltip) {
        ...
        const tooltipSnippet = `
            <div class="text-left">
                <h5>${i18n.msg(this.attrDef.label)} <code>${this.attrDef.key}</code></h5>
                ${tooltip}
            </div>
        `;
        return html`
    ...
    renderComponent() {
        ...
        const inputCss = `${iColSize ? `col-${iColSize}` : 'col'} ${this.getAddtionalDataCss()}`; // default

        return html` <div class="form-group row">
            <label class=${labelCss}
                >${i18n.msg(label)} ${mandatoryText} ${labelSuffixFn ? labelSuffixFn() : ''}${labelBeforeEndHTML}
